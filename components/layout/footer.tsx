...
export function Footer() {
  ...
  return (
    <footer className="relative bg-white !pb-0 !mb-0">
      {/* Main Footer Content */}
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          >
            ...
            <p className="text-black mb-8 leading-relaxed max-w-md">
            ...
            <div className="space-y-3 mb-8">
              {contactInfo.map((item, idx) => (
                <motion.div 
                  key={idx}
                  whileHover={{ x: 4 }}
                  className="flex items-start gap-3 text-black group"
                >
                  <span className="text-black mt-0.5 group-hover:scale-110 transition-transform">
                    {item.icon}
                  </span>
            ...
            <div className="flex gap-3">
              {socialLinks.map((social, i) => (
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="p-3 bg-gray-200 hover:bg-black border border-gray-300 rounded-lg text-black hover:text-white transition-all duration-300"
                >
                  {social.icon}
                </motion.a>
              ))}
          ...
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {footerSections.map((section, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
              >
                <h3 className="font-semibold text-black mb-6 text-lg">{section.title}</h3>
                <ul className="space-y-3">
                  {section.links.map((link, j) => (
                    <motion.li key={j} whileHover={{ x: 4 }}>
                      <Link 
                        href={link.href} 
                        className="flex items-center gap-2 text-black hover:text-orange-600 transition-all group text-sm"
                      >
      ...
      <div className="border-t border-gray-300 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-3">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            >
              <Link href="#" className="text-black hover:text-orange-600 transition-colors">Privacy Policy</Link>
              <Link href="#" className="text-black hover:text-orange-600 transition-colors">Terms of Service</Link>
              <Link href="#" className="text-black hover:text-orange-600 transition-colors">Cookies</Link>
            </motion.div>
            ...
            >
              ...
              <Link
                ...
                className="flex items-center gap-1 text-orange-600 hover:text-orange-400 font-medium transition-colors group"
              >
                Dmiraki
