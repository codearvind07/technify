...
import { useLayoutEffect, useRef, useCallback, CSSProperties } from 'react';
import Lenis from 'lenis';

export const ScrollStackItem = ({ children, itemClassName = '' }: { children: React.ReactNode; itemClassName?: string }) => (
  <div
    className={`scroll-stack-card relative w-full h-80 my-8 p-12 rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform transition-all duration-300 hover:shadow-[0_0_40px_rgba(0,0,0,0.15)] ${itemClassName}`.trim()}
    style={{
...
const ScrollStack = ({
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        wrapper: scroller,
        content: contentElement instanceof HTMLElement ? contentElement : undefined,
        duration: 1.2,
        easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 2,
