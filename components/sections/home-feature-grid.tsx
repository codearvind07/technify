...
const FeatureCard: React.FC<{ feature: Feature; className?: string; iconSize?: number }> = ({
  ...
  >
    <div className="flex flex-col items-center text-center p-6 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur-xl shadow-lg transition-all duration-500 group-hover:bg-white group-hover:border-orange-300/60">
      <motion.div
        className="mb-4 p-3 rounded-xl bg-gradient-to-br from-blue-50 to-orange-50"
        whileHover="hover"
        initial="initial"
      ...
      <SplitText
        text={feature.label}
        className="text-lg font-semibold text-gray-800 group-hover:text-orange-500 transition-colors duration-300"
        delay={0.1}
        duration={0.5}
        ease="power3.out"
        splitType="words"
...
export function HomeFeatureGridSection() {
  return (
    <section className="relative w-full py-16 md:py-24 bg-gradient-to-br from-blue-50 via-white to-orange-50 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Header */}
        <motion.header variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500/15 to-orange-500/15 backdrop-blur-md border border-blue-500/30 rounded-full px-6 py-2 mb-6 shadow-sm">
          ...
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold mb-4 text-gray-800 leading-tight">
          ...
          <SplitText
            ...
            className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto font-light leading-relaxed"
            splitType="words"
          />
        </motion.header>

        {/* Grid - Modified for better mobile responsiveness */}
        <main className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start mb-20">
          {/* Left */}
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="space-y-6">
            {leftFeatures.map((f, i) => (
              <FeatureCard key={f.label} feature={f} className={i === 1 ? "md:translate-x-4" : ""} />
            ))}
          </motion.div>
          ...
          <motion.div variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="flex flex-col items-center">
            <motion.div
              ...
              transition={{ duration: 3, repeat: Infinity, repeatType: "reverse" }}
            >
              <div className="relative w-44 h-44 md:w-64 md:h-64 rounded-3xl overflow-hidden border border-blue-200 shadow-2xl mx-auto">
            ...
            <FeatureCard feature={centerFeature} iconSize={48} />
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="mt-6 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold px-6 py-3 rounded-lg shadow-md hover:shadow-xl transition"
            >
              View All Solutions
            </motion.button>
          </motion.div>
        ...
        <motion.footer variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center max-w-4xl mx-auto">
          <SplitText text="Integrated Technology Solutions" className="text-2xl md:text-4xl font-bold text-gray-800 mb-6" splitType="words" />
          <SplitText
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {["System Integration", "24/7 Support", "Scalable Design", "Security First"].map((item, i) => (
              <div
                key={i}
                className="px-5 py-2 bg-gradient-to-r from-blue-500/15 to-orange-500/15 rounded-full text-gray-800 text-sm md:text-base font-medium shadow-sm hover:shadow-md transition"
              >
